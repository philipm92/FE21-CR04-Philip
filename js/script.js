...
function IncreaseLikes(index) {
    let ls_like_name = movie_list[index]["moviename"] + "_like_storage";
    movie_list[index]["likes"] = Number(movie_list[index]["likes"]) + 1;
    localStorage.setItem(ls_like_name, JSON.stringify(movie_list[index]["likes"]));
    document.getElementsByClassName("own-circle-size")[index].innerHTML = localStorage.getItem(ls_like_name);

    console.log(localStorage.getItem(ls_like_name), movie_list[index]["moviename"]);
}
...
var like_btn = document.getElementsByClassName("ownLikeClick");
